...
class GameStateManager:
            ...
            try:
                ...
                if len(primary_snapshots) > 4:
                    ...
                    if did_p1_primary_attack_change:
                        primary_old_id = primary_snapshots[-3].p1.movement_block.movelist_id
                        self.count_time_in_move_id(self.time_spent_in_primary_move_id_count[0], primary_old_id, primary_snapshots, True)
                    ...
                    if did_p2_primary_attack_change:
                        primary_old_id = primary_snapshots[-3].p2.movement_block.movelist_id
                        self.count_time_in_move_id(self.time_spent_in_primary_move_id_count[1], primary_old_id, primary_snapshots, False)
                ...
                if len(secondary_snapshots) > 4 and self.game_reader.game == Game.SCV:
                    ...
                    if did_p1_secondary_attack_change:
                        secondary_old_id = secondary_snapshots[-3].p1.movement_block.movelist_id
                        self.count_time_in_move_id(self.time_spent_in_secondary_move_id_count[0], secondary_old_id, secondary_snapshots, True)
                    ...
                    #did_p2_attack_change = snapshots[-1].p2.global_block.last_attack_address != snapshots[-2].p2.global_block.last_attack_address
                    did_p2_secondary_attack_change = primary_snapshots[-2].p2.movement_block.movelist_id != primary_snapshots[-3].p2.movement_block.movelist_id
    ...
    def create_frame_entry(self, name, p, record, bhc_stuns, times, movelist,verbose=False):
        ...
        if movelist != None:
            ...
            if (id >= start and id <= end) or (id >= MovelistParser.decode_move_id(0x3000, movelist) and id <= MovelistParser.decode_move_id(0x3000 + movelist.block_T_length, movelist)) or id == 219 or id == 221: #219 is soul charge, the only interesting move below 0x0100
                    if len(bhc_stuns) > 1:
                        stun = bhc_stuns[-1] #declare here in case we add a new one in FrameStringFromMovelist
                    s = GameStateManager.FrameStringFromMovelist(name, p, record, bhc_stuns, self.game_reader.primary_snapshots[-1].timer,verbose)
