...
class GUI_Main(Tk):
    def __init__(self):
        ...
        self.game_var = StringVar(None)
        self.game_menu.add_radiobutton(label="None", variable=self.game_var, command=self.set_game)
        self.game_menu.add_radiobutton(label="SCIV", variable=self.game_var, command=self.set_game)
        self.game_menu.add_radiobutton(label="SCV", variable=self.game_var, command=self.set_game)

        self.bind('<Control-Key-1>', lambda x: self.set_game('SCIV'))
        self.bind('<Control-Key-2>', lambda x: self.set_game('SCV'))
        self.bind('<Control-Key-`>', lambda x: self.set_game('None'))
        self.menu.add_cascade(label="Set Game", menu=self.game_menu)
        
        self.frame_config = ConfigReader.ConfigReader('frame_data_overlay')
        config = self.frame_config.get_property('DisplaySettings','game', None)
        self.game_var.set(config)
        self.set_game()
    ...
    def download_latest_release(self):
        webbrowser.open('https://github.com/LogzatioStudios150/Scuffle_RPCS3/releases/latest')

    def set_game(self, game = None):
        if game != None:
                self.game_var.set(game)
        if self.game_var.get() == 'None':
            self.launcher.game_reader.game = None
            print(f"No game selected")
        else:
            self.launcher.game_reader.game = Game[self.game_var.get()]
            print(f"Game has been set to {self.game_var.get()}")

        try:
            self.frame_config.set_property('DisplaySettings', 'game', self.game_var.get())
