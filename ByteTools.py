...
def GetValueFromAddress(processHandle, address, isFloat=False, is64bit=False, isString=False, is_short=False, length=16):
    error_code = -1

    if isString:
        data = c.create_string_buffer(length)
        bytesRead = c.c_ulonglong(length)
    elif is64bit:
        data = c.c_ulonglong()
    ...
    if is_short:
        return int.from_bytes(int.to_bytes(data.value, c.sizeof(data), byteorder='little'),'big')
    elif isString:
        try:
...
def GetDataBlockAtEndOfPointerOffsetList(process_handle, starting_pointer, offsets, size_of_block):
    ...
    for i in range(len(offsets)):
        if i < len(offsets):
            current_pointer = GetValueFromAddress(process_handle, current_pointer + offsets[i])
        current_pointer = starting_pointer + current_pointer
